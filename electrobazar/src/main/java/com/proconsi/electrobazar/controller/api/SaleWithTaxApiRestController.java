 ...
 *   <li>A detailed {@link SaleWithTaxResponse} is returned with per-line breakdowns
...
public class SaleWithTaxApiRestController {
    ...
    /**
     * Processes a sale with full VAT and optional Recargo de Equivalencia calculation.
     *
     * <p>The endpoint:</p>
     * <ol>
     *   <li>Resolves the customer (if provided) and checks RE eligibility.</li>
     *   <li>For each line, resolves the active price from the temporal pricing system.</li>
     *   <li>Calculates the tax breakdown per line using {@link RecargoEquivalenciaCalculator}.</li>
    ...
    public ResponseEntity<SaleWithTaxResponse> processSaleWithTax(
        ...
        for (SaleWithTaxRequest.SaleLineRequest lineReq : request.getLines()) {
            ...
            // Resolve price: use override if provided, otherwise use temporal pricing system
            BigDecimal unitPrice;
            BigDecimal vatRate;

            if (lineReq.getOverridePrice() != null) {
                unitPrice = lineReq.getOverridePrice();
                // For overridden prices, use the product's current active price VAT rate if available
                ...
                if (activePrice != null) {
                    ...
                    log.debug("Using temporal price {} (id={}) for product '{}' (id={})",
                            unitPrice, activePrice.getId(), product.getName(), product.getId());
                } else {
        ...
        // ── 5. Persist the sale ────────────────────────────────────────────────
        // Note: The Sale entity stores the grand total (including all taxes).
        // The detailed tax breakdown is returned in the response DTO.
        // We override the total in the sale lines to use the grand total per line.
        for (int i = 0; i < saleLines.size(); i++) {
            SaleLine line = saleLines.get(i);
            TaxBreakdown breakdown = taxBreakdowns.get(i);
            // Store the total line amount (including taxes) as the unit price for the sale record
            // This ensures the sale total matches the tax-inclusive amount
            line.setUnitPrice(breakdown.getTotalAmount()
                    .divide(BigDecimal.valueOf(breakdown.getQuantity()), MONETARY_SCALE, ROUNDING_MODE));
        }
     ...
     *
     * <p>Example: {@code POST /api/sales/with-tax/preview}</p>
     *
